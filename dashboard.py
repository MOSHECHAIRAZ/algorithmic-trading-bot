    ...
    with col1:
        ...
        if api_proc and api_proc.poll() is None:
            ...
            if st.button("▶️ הפעל שרת ה-API"):
                ...
                st.rerun()
    with col2:
        st.metric("סוכן מסחר", get_process_status(agent_proc))
...
# --- עמוד תצורת מערכת ---
    ...
    with st.form(key='config_form'):
        ...
        with st.expander("פרמטרי אימון (Training)", expanded=False):
            ...
            edited_config['training_params']['n_startup_trials'] = st.number_input("Optuna - ניסיונות אקראיים ראשוניים", min_value=0, value=edited_config['training_params'].get('n_startup_trials', 10), help="כמה ניסיונות ראשונים יהיו אקראיים לפני שהאופטימיזציה החכמה מתחילה")
            edited_config['training_params']['cv_splits'] = st.number_input("מספר קפלים (CV Splits)", min_value=2, value=edited_config['training_params'].get('cv_splits', 5))
            optuna_target_options = {
                'multi_objective': 'רב-יעדי (תשואה + יחס שארפ)',
                'total_return': 'תשואה כוללת (יעד בודד)',
                'sharpe_ratio': 'יחס שארפ (יעד בודד)'
            }
            ...
            optuna_target = st.selectbox("יעד אופטימיזציה (Optuna)", options=list(optuna_target_options.keys()), format_func=lambda k: optuna_target_options[k], index=optuna_target_index)
            edited_config['training_params']['optuna_target_metric'] = optuna_target
            test_size_percent = st.number_input("אחוז סט בדיקה (10-50%)", min_value=10, max_value=50, value=int(edited_config['training_params'].get('test_size_split', 20)), step=1)
            edited_config['training_params']['test_size_split'] = test_size_percent
            edited_config['training_params']['years_of_data'] = st.number_input("שנות נתונים לאימון", min_value=1, max_value=30, value=edited_config['training_params'].get('years_of_data', 15))

        with st.expander("הגדרת גבולות לפרמטרי אופטימיזציה (Optuna)", expanded=True):
            optuna_limits = edited_config.get('optuna_param_limits', {})
            
            # 1. עדכון מילון התוויות
            param_labels = {
                'horizon': 'אופק תחזית (ימים)', 'threshold': 'סף החלטה (%)',
                'top_n_features': 'מספר פיצ׳רים', 
                'stop_loss_pct': 'עצירת הפסד (%)',
                'take_profit_pct': 'לקיחת רווח (%)',
                'risk_per_trade': 'סיכון לעסקה (%)',
                'learning_rate': 'קצב למידה', 'n_estimators': 'מספר עצים', 'max_depth': 'עומק מירבי'
            }
            ...
            for param, lims in optuna_limits.items():
                ...
                # 2. עדכון רשימת האחוזים
                is_percent = param in ['threshold', 'risk_per_trade', 'stop_loss_pct', 'take_profit_pct']
                ...
                with col3:
                    new_dynamic_state = st.toggle(
                        "אופטימיזציה", 
                        value=is_dynamic, 
                        key=f"toggle_{param}", 
                        help="הפעל כדי לחפש בטווח, כבה כדי לקבוע ערך קבוע."
                    )
                
                # 3. הוספה מחדש של שדות הקלט ותיקון הלוגיקה
                with col1:
                    help_text = "ערך קבוע (כשהאופטימיזציה כבויה) או הגבול התחתון (כשהיא פועלת)."
                    # הערכים מוצגים ונקלטים באחוזים אם צריך
                ...
                with col2:
                    user_max = st.number_input(
                        ...
                        value=float(max_val), 
                        format="%.4f", 
                        disabled=not new_dynamic_state,
                        help="הגבול העליון (רלוונטי רק כשהאופטימיזציה מופעלת)."
                    )
        ...
        with st.expander("פרמטרי Backtest"):
        ...
        with st.expander("פרטי חוזה ונכס (Contract)"):
            edited_config['contract']['symbol'] = st.text_input("סימבול", value=edited_config['contract'].get('symbol', 'SPY'))

        with st.expander("הגדרות חיבורים (API & IBKR)"):
            edited_config['api_settings']['port'] = st.number_input("פורט שרת API", min_value=1024, max_value=65535, value=edited_config['api_settings'].get('port', 5000))
            ...
            edited_config['ibkr_settings']['history_window'] = st.text_input(
                "חלון זמן לנתונים בסוכן (IBKR)",
                value=edited_config['ibkr_settings'].get('history_window', '90 D'),
                help="כמה נתונים היסטוריים הסוכן יבקש לפני קבלת החלטה. פורמט: 'X D' (ימים), 'X W' (שבועות), 'X M' (חודשים). למשל: '90 D'."
            )
                    ...
                    with st.expander(expander_title):
                        ...
                        with exp_col1:
                            if candidate_model_path.exists() and candidate_config_path.exists():
                                if st.button(f"📊 הרץ בקטסט", key=f"bt_{ts}"):
