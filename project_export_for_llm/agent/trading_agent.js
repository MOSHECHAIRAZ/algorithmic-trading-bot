...
const CONFIG = (() => {
    try {
        ...
        return {
            modelApiUrl: `http://${process.env.API_HOST || config.api_settings?.host || 'localhost'}:${process.env.API_PORT || config.api_settings?.port || 5000}`,
            logFile: path.join(__dirname, 'trading_log.txt'),
            ib: {
                host: process.env.IB_HOST || config.ibkr_settings?.host || '127.0.0.1',
                port: parseInt(process.env.IB_PORT || config.ibkr_settings?.port || '4001', 10),
                clientId: parseInt(process.env.IB_CLIENTID || config.ibkr_settings?.clientId || '101', 10),
            },
...
async function syncWithPortfolio(ibContract) {
    ...
    try {
        // Get actual positions from account
        const allPositions = await getRealPositions();
        ...
        log(`Current local state: Position=${state.position || 'None'}, Size=${state.size || 0}`);
        
        // Case 1: Real position exists in portfolio
        if (realPosition) {
            const realSize = realPosition.position;
            const realAvgCost = realPosition.avgCost;
            
            log(`Found REAL position in portfolio: ${realSize} shares of ${realPosition.contract.symbol} at avg cost ${realAvgCost}.`);
            
            // Determine if local state needs updating
            if (state.position !== 'long' || state.size !== realSize || !state.entryPrice) {
                log('Local state is out of sync with real portfolio. Correcting it now.', 'warn');
                
                // Update with real position data
                state = {
                    ...state, // preserve other fields like parentOrderId if they exist
                    position: 'long',
                    size: realSize,
                    entryPrice: realAvgCost,
                    lastSyncTime: new Date().toISOString()
                };
                
                await setState('trade_state', state);
                log(`State updated to match real portfolio position`);
            } else {
                log(`Local state is in sync with real portfolio. No changes needed.`);
            }
        } 
        // Case 2: No real position in portfolio
        else {
            log(`No real position for ${ibContract.symbol} found in portfolio.`);
            
            if (state.position === 'long') {
                log('Local state shows a position, but none exists in portfolio. Clearing position data from local state.', 'warn');
                
                // Create a new state object, preserving any fields not related to position
                const newState = { ...state };
                delete newState.position;
                delete newState.entryPrice;
                delete newState.size;
                delete newState.parentOrderId;
                newState.lastSyncTime = new Date().toISOString();
                
                await setState('trade_state', newState);
                log(`Position data cleared from local state`);
                
                // Update our working copy of state
                state = newState;
            } else {
                log(`Local state correctly shows no position. No changes needed.`);
            }
        }
        
        log(`Reconciled state is: Position=${state.position || 'None'}, Size=${state.size || 0}`);
        return state;
    } catch (error) {
        log(`Error during portfolio synchronization: ${error.message}`, 'error');
        log(`Stack trace: ${error.stack}`, 'error');
        // Return the current state without changes in case of error
        return await getState('trade_state') || { position: null };
    }
}
...
async function startAgent() {
    ...
    const connectWithRetry = async () => {
        try {
            log(`Connecting to IB API (attempt ${connectionAttempts + 1}/${MAX_RECONNECT_ATTEMPTS})...`);
            ib.connect();
            
            // Reset connection attempts on successful connection
            ib.on(EventName.connected, async () => {
                connectionAttempts = 0;
                log("IB API Connected successfully.");
            ...
            ib.on(EventName.disconnected, () => {
                log("IB API Disconnected unexpectedly.", "warn");
                
                if (connectionAttempts < MAX_RECONNECT_ATTEMPTS) {
                    connectionAttempts++;
                    log(`Will attempt to reconnect in ${RECONNECT_DELAY_MS/1000} seconds...`, "info");
                    setTimeout(connectWithRetry, RECONNECT_DELAY_MS);
                } else {
                    log("Maximum reconnection attempts reached. Giving up.", "error");
                }
            });
        } catch (err) {
    ...
    ib.on(EventName.error, (err, code, reqId) => {
        ...
        if (code === 504 || code === 502 || code === 1100) {
            log("Critical connection error detected.", "error");
            if (connectionAttempts < MAX_RECONNECT_ATTEMPTS) {
                connectionAttempts++;
                log(`Will attempt to reconnect in ${RECONNECT_DELAY_MS/1000} seconds...`, "info");
                setTimeout(connectWithRetry, RECONNECT_DELAY_MS);
...
process.on('SIGINT', async () => {
    ...
    try {
        ...
        if (ib.isConnected()) {
            log("Disconnecting from IB API...");
            await ib.disconnect();
        }
        log("Trading agent shutdown complete. Exiting.");
        process.exit(0);
    } catch (err) {
